"""
Scraper de MercadoLibre Inmuebles - VENTA Residencial en CABA (usados)
======================================================================
TP Analítica Descriptiva - 1ra Pre-Entrega
Temática: Inteligencia de Mercado Inmobiliario (Real Estate Analytics)

Adaptado del scraper de alquiler del equipo. Dos cambios de fondo:
  - Operación: /venta/ en lugar de /alquiler/.
  - Filtro de USADOS (_ITEM*CONDITION_2230581): en venta, sin este filtro
    ML mezcla EMPRENDIMIENTOS (obra nueva, precio "Desde", m² en rangos),
    que no sirven para el análisis. La paginación (_Desde_N) se inserta
    ANTES del filtro en la URL.
Y dos columnas nuevas: 'operacion' (=venta) y 'es_emprendimiento' (red de
seguridad por si algún emprendimiento se cuela pese al filtro).

Qué hace:
  Recorre el mercado de VENTA residencial en CABA en MercadoLibre
...
OUTPUT_CSV = Path("mercadolibre_venta_caba.csv")

# --- MODO: qué universo traer en esta corrida ---
#   "usados"          -> propiedades usadas (obra existente)
#   "emprendimientos" -> obra nueva / desarrollos (precio "Desde", en rangos)
# Corré el script una vez en cada modo; ambos escriben al MISMO CSV,
# etiquetando cada aviso con es_emprendimiento. Deduplica por id_aviso.
MODO = "emprendimientos"

# Sufijos de filtro de ML. La paginación (_Desde_N) va ANTES de estos sufijos.
FILTROS = {
    "usados": "_ITEM*CONDITION_2230581_NoIndex_True",
    # PENDIENTE: pegá acá el sufijo del filtro de emprendimientos/nuevo.
    # Lo sacás de la URL cuando aplicás ese filtro en el navegador (ver instrucciones).
    "emprendimientos": "_ITEM*CONDITION_2230284_NoIndex_True",
}
FILTRO = FILTROS[MODO]
OPERACION = "venta"
...
CATEGORIAS_SIN_SEGMENTAR = {
    "ph": f"{BASE_URL}/ph/venta/capital-federal/",
    "casa": f"{BASE_URL}/casas/venta/capital-federal/",
}
...
FIELDNAMES = [
    "id_aviso", "titulo", "operacion", "es_emprendimiento", "direccion", "barrio",
    "ciudad", "moneda", "precio", "m2_cubiertos", "dormitorios", "banos",
    "ambientes", "tipo_propiedad", "link",
]
...
def item_to_row(item, tipo_propiedad):
    ...
    # Etiqueta de emprendimiento: por detección ("Desde") y reforzada por el modo
    es_emp = 1 if (price_raw and "desde" in price_raw.lower()) else 0
    if MODO == "emprendimientos":
        es_emp = 1

    return {
        "id_aviso": parse_id_aviso(link), "titulo": titulo, "operacion": OPERACION,
        "es_emprendimiento": es_emp, "direccion": location_raw,
...
def scrape_segmento(session, base_url, tipo_propiedad, seen_ids, nombre):
    ...
    for pagina in range(MAX_PAGINAS):
        ...
        # La paginación (_Desde_N) va ANTES del sufijo del filtro del modo
        if pagina == 0:
            url = f"{base_url}{FILTRO}"
        else:
            url = f"{base_url}_Desde_{offset}{FILTRO}"
...
def scrape():
    if "PEGAR_SUFIJO" in FILTRO:
        print(f"[ERROR] El filtro para el modo '{MODO}' todavía no está configurado.")
        print("        Aplicá ese filtro en el navegador y pegá el sufijo de la URL en FILTROS.")
        return None
    ...
    for i, barrio in enumerate(BARRIOS, 1):
        url = f"{BASE_URL}/departamentos/venta/capital-federal/{barrio}/"
